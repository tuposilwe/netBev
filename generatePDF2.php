...
$sql= "SELECT * FROM main_delivery WHERE invoice_no=( SELECT MAX(invoice_no) FROM  main_delivery)";
...
$pdf->Cell(0, 10, 'please receive the following goods:', 0, 1, 'L');
...
// Set column widths
$colWidths = array(40, 90); // Adjust column widths as needed

// Header row
$pdf->Cell($colWidths[0], 10, 'Quantity', 1, 0, 'C');
$pdf->Cell($colWidths[1], 10, 'Particulars', 1, 1, 'C');

// Retrieve data from the database
$sql = "SELECT quantity, particulars FROM delivery  WHERE invoice_no=( SELECT MAX(invoice_no) FROM main_delivery)";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell($colWidths[1], 10, $row['particulars'], 1, 1, 'C');
...
$pdf->Cell(0, 10, 'all the above goods were received in a good order and condition', 0, 0);
